...
前提: **大学のプロジェクト。無料で公開し、課金は実装しない。** ここで見るのは「どの機能領域が埋まっていて、どこが空いているか」だけ。

## 結論(先に)

1. **記録(GPSロガー)の土俵では勝てない。勝つ必要もない。** 日本市場は評価数 1,000〜7,000 件規模の先行アプリが機能を出し尽くしている(GPX 入出力・ヒートマップ・iCloud 同期・ウィジェット)。
...
| アプリ | 評価 | 特徴 |
|---|---|---|
| ルートヒストリー | ★4.5 / 7,122件 | GPX 入出力、ログ結合、ルート再生。記録の完全性で勝負 |
| ROADSTOCK | ★4.6 / 1,414件 | ツーリング記録+給油/整備/出費。バイク寄り。**画像生成なし** |
| Drive Tracker | ★4.3 / 12件 | 2026 年の新顔。ヒートマップ・ウィジェット・Siri・CSV |
| DriveHistory / Drive Report | — | 走行履歴の可視化・走行距離管理 |

読み: 機能競争に入ると評価数の差で負ける。**記録は「Story を作るための入力」であって、それ自体を主要訴求にしない。**

## レイヤー2: 変換(ここを取る)

| サービス | 規模 | 何を変換するか |
|---|---|---|
| Relive | 2,200万ユーザー | GPS 活動 → 3D 動画ストーリー。写真をルート上に自動配置 |
| Strava Stats Stickers | — | 距離/獲得標高/時間+ルート線画 → IG ストーリー用ステッカー |
| Polarsteps | 1,000万人 | 旅程 → 自動タイムライン(経路+写真+統計) |
...
| サービス | 規模 | 読み |
|---|---|---|
| CARTUNE(国内) | 車 SNS 最大級 MAU | 投稿の宛先は既にここ。競合ではなく**出口**として使う |
| みんカラ(国内) | 老舗。ドライブモード(同時間・同場所の可視化) | 30〜50代中心。若年層は CARTUNE へ移行 |
| RoadStr | 10,000+ ルート・100k+ DL・★4.4 | ルート発見+イベント+クラブ。CarPlay 対応 |
| GarageApp | — | ルートプランナー+近所のオーナー検索 |
| Rever / Calimoto / Scenic(バイク) | — | ルート発見に特化。「曲がり具合」で経路を選ぶ設計 |
...
- [RoadStr](https://apps.apple.com/gb/app/roadstr-car-routes-events/id1382535778) / [GarageApp](https://garageapp.com/) / [Rever・Calimoto・Scenic(SlashGear)](https://www.slashgear.com/1755865/motorcycle-apps-find-routes-track-rides/)
