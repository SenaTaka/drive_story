...
- **自動記録のときだけ Always と背景実行を使う。** 当初は「Always を使わない（審査コストとバッテリーが割に合わない）」
  としていましたが、開くのを忘れて記録し損ねるのが体験上いちばん惜しいので方針を変えました（2026-08-29）。
  **既定はオフ**で、オンにしたときだけ背景で位置を取り、背景利用インジケータを出します。
- **走り出しの判定は 2 系統で持つ。** Core Motion の `.automotive` が実機の本命、GPS 速度の持続がフォールバック。
  片方が使えなくても機能を落としません。シミュレータは Core Motion が使えないので、速度側だけで動きます。
- **停止は速度ではなく変位で判断する。** 速度の申告が張り付いたまま更新されない端末があり、
  速度だけを信じると止まったことに気づけません（シミュレータで実測。走り終わっても 144km/h のままでした）。
...
- **PhotoKit を境界の外に置く。** 突き合わせは `PhotoCandidate`（識別子・撮影時刻・座標だけ）を受けます。
  `PHAsset` を直接受けると、写真権限のない環境でアルゴリズムを一切検証できなくなるためです。
  本番の入口は `PHAsset` を候補に写してから同じ判定に通すので、経路は変わりません。
...
（マスク圏内 / ルートから遠い / 時刻レンジ外 / 位置なし を必ず混ぜる）。**この検証は実装済みですが、
シミュレータ基盤が不安定になったため実行までは至っていません。**
...
| 11 | 走り出しの自動検知（`DriveDetector`・背景実行） | `start()` を呼ばず検知だけで記録が始まり、止まったら締まる | 済 |
...
DriveStory/
  Record/     RoutePoint          走行軌跡の 1 点（記録用の生データ）
              ...
              DriveDetector       走り出しと停車の判定（Core Motion + GPS 速度）
  ...
  Photos/     PhotoRef            Story に載せる候補 1 枚（実画像は持たない）
              ...
              PhotoMatcher        走行時刻・位置と写真の突き合わせ（PhotoCandidate を受ける）
...
- **権限**: `xcrun simctl privacy <udid> grant location <bundle>` を、**install の後・アプリを一度も起動しないうちに**実行します。install より前だと効きません（TCC のエントリが install で流れます）。一度でも位置プロンプトを出した端末では grant が効かなくなるので、専用シミュレータを毎回作って捨てます。
- **写真の権限だけは機械で与えられません**: `grant photos` は iOS 26.0 でも 18.6 でもエラーを出さずに `notDetermined` のままでした（`grant all` も TCC.db 直書きも同じ）。**実 `PHAsset` の読み出しと写真ライブラリへの保存は実機でしか検証できません。** 突き合わせの判定そのものは `PhotoCandidate` 経由で権限なしに検証します。
...
- **アプリを起動プロセスに紐づけない**: `simctl launch --console-pty` を使うとアプリが launch プロセスの子になり、スクリプトが止まった瞬間に**アプリごと落ちて成果物が書かれません**。
- **`--auto` で自動記録を検証**: `start()` を呼ばず、検知だけで記録が始まり・締まることを確かめます。判定のロジックは本番のまま、待ち時間だけ環境変数で縮めます。
- **タップ不要**: `DRIVE_VERIFY=1` で起動すると、記録開始 → 走行 → 終了 → 写真マッピング → Story 生成までを画面操作なしで実行し、`Documents/verify/<runid>/` に走行 JSON・写真の採否理由・Story 4 枚・アニメのフレーム連番・判定結果を書き出します。どの段まで進んだかは `02_phase.log` に残るので、止まった場所が後から分かります。**ハーネスは UI 層だけをバイパスし、本番と同じサービス層を呼びます** — ハーネス専用のロジックを書いた瞬間、この検証は何も担保しなくなります。

回収したら **Story 4 枚とアニメのコンタクトシートは必ず開いて見てください。** 機械判定の pass は「真っ黒/白紙ではない」ことしか保証しません。

### この経路で検証できないもの

権限ダイアログの文言と初回体験 / ボタンのタップ反応と共有シート / アニメーションの質感 /
実 GPS の精度とトンネル欠測 / HEIC・Live Photo・iCloud 未ダウンロードの実カメラロール /
**実 `PHAsset` の読み出しと写真ライブラリへの保存**（写真権限を機械で与えられないため）/
**Core Motion による走り出しの検知**（シミュレータでは利用不可。速度側だけが動きます）/
背景での継続記録とバッテリー消費。これらは実機で人間が確認します。
